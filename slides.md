...
<!--
这里是选区交互的 状态机设计图。从 Idle 等待状态出发，通过鼠标与键盘事件（如 Esc 取消、Enter 确认、双击等），系统能够流畅的在各个状态间流转，保证了极高的稳定性。
-->
...
<!--
在开发这个模块的时候，
（Toolbar）在不同屏幕分辨率下的遮挡与适配问题
我发现当截图区域靠近屏幕边缘，或者在一些高分屏、缩放比例不是100%的电脑上运行时，由于工具栏坐标计算与实际物理分辨率不匹配，工具栏经常会飘到屏幕外面，或者直接挡住截图区域，导致无法进行标注。
这个问题我是通过动态定位解决的。工具栏会根据选区位置自动判断放在上下左右哪个方向最合适；如果发生溢出，就反向调整，并用边界把它限制在屏幕可用区域内，所以不同分辨率下都能保证工具栏不跑出屏幕、不被任务栏遮挡。”
-->
...
第一，Clipboard 的作用是提升即时使用体验。比如截图之后，用户不用再手动点复制，可以直接粘贴到微信、QQ、文档或者聊天窗口。
第二，PNG 自动保存是为了保证截图有本地备份。代码里会创建 Pictures/PrintScreenApp 目录，然后用时间戳生成文件名样，这每张截图都能追溯。
第三，Tray 托盘让程序常驻后台。因为截图工具一般不需要一直显示主窗口，只要在后台等待快捷键或托盘操作就可以。
最后，Registry 是开机自启的实现方式，程序会写入当前用户的 Run 项，这样电脑启动后截图工具也会自动启动。
-->
...
<!--
纵观整个系统，这是一张完整的架构拓扑图。程序从 Program 入口出发，利过主窗体 Form1 作为中央协调器，纵穿了基于 Win32 与钩子的系统支撑层，横调用了罩、选区交互为主的截图层，以向及面接向口标 注层，
-->
...
<!--
最后，是我们项目计划的迭代路线 。
目前我们已经完成了 v1.0 基于 WinForms 的核心功能闭环
在后续的规划中，v2.0 阶段，我们计划引入 WPF 现代化重构
随后，在v3.0 阶段，我们将实现基础的人机交互，引入智能悬停识别，让截图工具更加智能高效。
-->
